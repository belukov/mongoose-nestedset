...
module.exports = exports = function(schema, options)
{
  ...
  schema.pre('save', function(next) {
    ...
    if(this.isNew) {
      ...
      if(node.isModified('parentId')) {
        ...
        node.constructor.findById(node.parentId, function(err, newParent) {
        
          node.parentId = node._origin.parentId;
          node.move(newParent, function(err) {
            if(err) return next(err);
            node.parentId = newParent._id;
            return next();
          });
        });
  ...
  schema.method('move', function(newParent, next) {
    ...
    if('object' != typeof newParent || 'function' != typeof newParent.append) {
      // no nested node
      mdl.findById(newParent, function(err, newParent) {
        if(err) return next(err);
        return node.move(newParent, next);
      });
    ...
    mdl.shiftAfter(border, size, function(err) {
      ...
      mdl.spread(newParent, size, function(err) {
        ...
        //node.parentId = newParent._id,
        //node.nleft = border;
        //node.nright = border + size -1;
        //node.level = newParent.level + 1;
        //console.log('save', node.toObject());
