...
module.exports = exports = function(schema, options)
{
  ...
  schema.method('_move', function(newParent, next) {
    ...
    if(newParent) {
      ...
      //console.log("shift after ", border);
      mdl.shiftAfter(border, size, function(err) {
        if(err) return next(err);
        //console.log('spread', size);
        mdl.spread(newParent, size, function(err) {
          ...
          mdl.update(
            ...
            function(err, updated) {
              ...
              var diff = border - oldArea.nleft;
              if (diff < 0) { // was shifted before
                diff = diff - size;
                oldArea.nleft += size;
                oldArea.nright += size;
              }
              ...
              var upd = {
                $inc: {
                  level: newParent.level + 1 - oldArea.level,
                  nleft: diff,
                  nright: diff
                }
              };
              //console.log('update area ', upd);
              mdl.update({
                nleft: {$gte: oldArea.nleft},
                nright: {$lte: oldArea.nright},
              }, upd, {multi: true}, function(err, updated) {
                if(err) return next(err);
      ...
      mdl.findOne({}).sort('-nright').exec(function(err, last) {
        ...
        function(err, updated) {
          ...
          mdl.update(
            {
              nleft: {$gte: node.nleft},
              nright: {$lte: node.nright}
            },
            upd,
            {multi: true},
          function(err, updated) {
            if(err) return next(err);
            //console.log('updated to root ', updated);
            //console.log('unspread');
            mdl.findById(oldParentId, function(err, oldParent) {
              if(err) return next(err);
              mdl.unSpread(oldParent, size, function(err) {
  ...
  schema.method('parent', function(next) {
    var node = this;
    ...
    mdl.findOne(cond).exec(next);
  });

  schema.method('children', function(next) {
    var node = this;
    var mdl = node.constructor;
